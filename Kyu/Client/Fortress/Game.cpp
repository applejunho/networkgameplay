...
// PCH °æ°í(E2923) ÇØ°á: #pragma comment(lib, ...)´Â ÆÄÀÏÀÇ ¸Ç À§(Çì´õ Æ÷ÇÔ Àü)¿¡ À§Ä¡ÇØ¾ß ÇÔ
...
void Draw_tank()
{
    ...
    if (player2_left && isFired && player_2turn) {
        ...
        switch (player2TankNumber) {
        case 1: //1ë²ˆ íƒ±í¬ ì˜¤ë¥¸ìª½ idle
            SelectObject(MemDC, tank1_idle_right);
            GdiTransparentBlt(hBackBuffer, B.left, B.top, 37, 29, MemDC, 0, 0, 37, 29, RGB(255, 0, 255));
            break;
        case 2: //2ë²ˆ íƒ±í¬ ì˜¤ë¥¸ìª½ idle
            SelectObject(MemDC, tank2_idle_right);
            GdiTransparentBlt(hBackBuffer, B.left, B.top, 35, 21, MemDC, 0, 0, 35, 21, RGB(255, 0, 255));
            break;
        case 3: //3ë²ˆ íƒ±í¬ ì˜¤ë¥¸ìª½ idle
            SelectObject(MemDC, tank3_idle_right);
            GdiTransparentBlt(hBackBuffer, B.left, B.top, 32, 25, MemDC, 0, 0, 32, 25, RGB(255, 0, 255));
            break;
        }
...
void Draw_shoot()
{
    if (A.isFire)
    {
        if (A.shoot_mode == 1 && A.shoot1 == true) {
            ...
            if (bulletAni == 0)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 11, 9, MemDC, 0, 0, 11, 9, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 1)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 11, 11, MemDC, 0, 0, 11, 11, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 2)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 11, 11, MemDC, 0, 0, 11, 11, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 3)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 11, 11, MemDC, 0, 0, 11, 11, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            ...
            if (bulletAni == 0)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 18, 17, MemDC, 0, 0, 18, 17, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 1)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 18, 17, MemDC, 0, 0, 18, 17, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 2)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 17, 18, MemDC, 0, 0, 17, 18, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 3)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 18, 17, MemDC, 0, 0, 18, 17, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
        }
    }
        ...
        if (B.shoot_mode == 1 && B.shoot1 == true) {
            ...
            if (bulletAni == 0)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 9, 12, MemDC, 0, 0, 9, 12, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 1)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 13, 9, MemDC, 0, 0, 13, 9, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 2)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 9, 11, MemDC, 0, 0, 9, 11, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 3)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 18, 9, MemDC, 0, 0, 18, 9, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
        }
        else {
            if (B.shoot_mode == 2) {
                BitemTeleport = true;
            ...
            if (bulletAni == 0)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 13, 15, MemDC, 0, 0, 13, 15, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 1)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 17, 13, MemDC, 0, 0, 17, 13, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 2)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 13, 18, MemDC, 0, 0, 13, 18, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
            else if (bulletAni == 3)
                GdiTransparentBlt(hBackBuffer, x - 3.5, y - 3.5, 23, 13, MemDC, 0, 0, 23, 13, RGB(255, 0, 255)); // ¼öÁ¤ÇÑ ºÎºÐ
        }
    }
}
    ...
    if (A.isFire == true)
    {
        if (y > 0 && x > 0 && x < 1600 && y < 800)
        {
            if (GetPixelColor(hBitmap, x + 2.5, y + 2.5) != TARGET_COLOR)
            {
                ...
                // ¡Ú ³»°¡ A(0¹ø ÇÃ·¹ÀÌ¾î)¶ó¸é ÅÏ Á¾·á ÆÐÅ¶ Àü¼Û
                ...
                // ¡Ú °ÔÀÌÁö/½ºÆäÀÌ½º »óÅÂ ÀüºÎ ÃÊ±âÈ­
                A.ResetGauge();
                B.ResetGauge();

                // Æ÷Åº À§Ä¡ ÃÊ±âÈ­ (ÇÊ¿äÇÏ´Ù¸é µÑ ´Ù)
                A.set_ball();
                // B.set_ball();  // ÇÊ¿ä ¾øÀ¸¸é »ý·«
...
void wind(HWND hWnd)
{
    ...
    Fire_turn(); // ÅºÈ¯ ÃßÀû Ä«¸Þ¶ó
    camera_turn(); // ÇÃ·¹ÀÌ¾î ÃßÀû Ä«¸Þ¶ó
}
...
void DrawFrame(HWND hWnd, HDC hDC)
{
    ...
    if (isCharacter1Selected && isCharacter2Selected && isMapSelected)
    {
        ...
        // ¸Þ¸ð¸® Á¤¸® (ÁÖÀÇ: hDC´Â DeleteDC ÇÏ¸é ¾ÈµÊ!)
        DeleteObject(hBackBufferBitmap);
        DeleteDC(hBackBuffer);
        DeleteDC(MemDC);
